...
class HomeSuggestions(APIView):
    def get(self, request):
        ...
        for m in movies:
            if c==3 :
                break
            if m.imgurl == None or m.imgurl == "":
                continue
            c+=1
            l.append({"id" : m.id, "name" : m.title, "imgurl" : m.imgurl, "caststr" : m.caststr(), "year" : m.year, "runningtime" : m.runningtime, "directorstr" : m.directorstr(), "aggrating" : m.aggrating})
